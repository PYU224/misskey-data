...
## インスタンス内のコミュニティ傾向
コミュニティは流動的な物なので、あくまでも一例です。
- IT技術に長けたユーザーが多いです。
- 創作をしているユーザーも見受けられます。
- プログラミング、小説執筆、イラスト作成(アナログ・デジタル問わず)、作曲、演奏など幅広いクリエイティブ行為をしているユーザーが居ます。
- 他のインスタンスと比べると「閉じた」インスタンスになっています。
- 基本的には同じインスタンス内で共通の話題で会話する事が多いです。
- 年齢層はやや高め(30代前後が中心)になります。
- 例のアレ系のコミュニティ出身のユーザーも見受けられます。
...
- 差別的発言は規制対象になります。自由と平等のためには一定の制約が必要という事は頭の片隅に置いてください。
- 本インスタンスは独立系インスタンスであり、どこかの避難所としての利用を主とする方の利用はお断りさせていただきます。
- 他にも色々と追加するかも
<br>
...
<p>色々と制限を付けましたが創作や技術についての話題、特にプログラミング・サーバといった話題については大歓迎です。</p>

## セキュリティに関して
- 本インスタンスは過去に脆弱性があった(現在は修正済)ため、メールアドレス認証だけでなく認証アプリなどを用いた2段階認証を推奨しています。
- メールアドレス認証をしていただくと、パスワードリセットも使えるため設定を推奨します。(無くても利用は可能です)

## 他にも気を付けて欲しい事とか諸々

[揉め事を起こさないためのアドバイス（advice.md）](./advice.md)
...
## 寄付について
<p>このインスタンスが私PYUが運営しているインスタンスです。<br>
企業等のバックアップや広告収益がある訳ではなく、管理人が全額負担しています。</p>
<p>サーバの運営や勉強代の為に寄付を常時募集しています<br>
...
https://33-4.me/blog/archives/797<br>
<br>
...
このサーバーをホストしているVPS（仮想サーバー）はモルドバ共和国🇲🇩にあるホスティング会社のものを利用しています。<br>
理由は比較的安価で規約が緩くて性能の良いサーバをレンタルでき、VPSのレンタルを通じて真面目で勤勉な彼らの支援がしたいからです。<br>
ただ最近はエネルギー問題で高くなってきていりますが。
...
- ヴァーレントゥーガまとめwiki（ここのシステムを使ってゲームを作っています）
https://w.atwiki.jp/vahren_ency/pages/1.html
